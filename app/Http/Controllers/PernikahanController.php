...
class PernikahanController extends Controller
{
    ...
    public function indexUmat()
    {
        ...
        $link = route('sidi.umat');
        if ($sidi === null) {
            $disableBtn = false;
            return view('umat.pernikahan', compact('data', 'disableBtn'))
                ->withErrors(['error' => [
                    'Anda tidak dapat melakukan pendaftaran pernikahan',
                    'Anda belum melakukan pendaftaran katekisasi',
                    "Silahkan anda melakukan pendaftaran katekisasi terlebih dahulu, berikut link <a href=\"$link\">Pendaftaran Katekisasi</a>",
                    'Pastikan pasangan anda sudah memenuhi persyaratan untuk melakukan pendaftaran pernikahan',
                ]]);
        }
    ...
    public function print()
    {
        $date = now();
        $data = $this->getData();
        return view('umat.pernikahanPrint', compact('data','date'));
    }
