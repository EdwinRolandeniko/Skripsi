...
class DashboardController extends Controller
{
    ...
    public function indexUmat()
    {
        ...
        $sidi = Sidi::where('id_user', $id)->get();
        $gender = auth()->user()->gender;
        if($gender == 'L'){
            $pernikahan = Pernikahan::where('status', 'DITERIMA')->where('id_user_laki', $id)->exists();
        }else if($gender == 'P'){
            $pernikahan = Pernikahan::where('status', 'DITERIMA')->where('id_user_perempuan', $id)->exists();
        }
